...
# Procurement Controller - Handles RFQs and vendor communication
...
class Vmcott::Procurement::DashboardController < ApplicationController
  ...
  def index
    ...
    @pending_parts_requests = PartsRequest.where(status: 'procurement_notified')
    ...
    # Parts received (for stock update) - for inventory manager
    @parts_received = Inspection.joins(:parts_requests)
                            .where(parts_requests: { status: 'parts_received' })
                            .where(status: 'inventory_manager_review')
                            .distinct
                            .includes(:vehicle, :inspection_jobs, :parts_requests)
                            .order(updated_at: :desc)

    # KPI counts for dashboard
    @parts_to_quote_count = PartsRequest.where(status: 'procurement_notified').count
    ...
    @pending_forward_count = VendorRfq.where(finance_review_ready: false)
    ...
    # Stats for KPI cards
    @stats = {
      parts_to_quote: @parts_to_quote_count,
      active_rfqs: @active_rfqs_count,
      quotes_received: @quotes_received_count,
      pending_forward: @pending_forward_count
    }
  end
  ...
  def new_rfq
    ...
    if @parts_request.nil?
      redirect_to vmcott_procurement_dashboard_path, alert: "Parts request not found."
      return
    end
  ...
  def create_rfq
    ...
    # Validate suppliers are selected
    if params[:supplier_ids].blank?
      flash[:alert] = "Please select at least one supplier."
      redirect_to vmcott_procurement_new_rfq_path(parts_request_id: parts_request.id) and return
    end
    ...
    if rfq.save
      ...
      parts_request.update(status: 'rfq_sent', notified_procurement_at: Time.current)
    ...
    redirect_to vmcott_procurement_dashboard_path, alert: "Error: #{e.message}"
  end
  ...
  def forward_to_finance
    ...
    if rfq.vendor_quotations.where(status: 'received').empty?
      redirect_to vmcott_procurement_dashboard_path, alert: "Cannot forward: No quotations have been received yet."
      return
    end
    ...
    if rfq.update(finance_review_ready: true)
      ...
      redirect_to vmcott_procurement_dashboard_path, notice: "Quotations forwarded to finance team."
    else
  ...
  def require_procurement
    unless current_user.procurement? || current_user.admin?
      redirect_to root_path, alert: "Access denied. Procurement privileges required."
    end
  end
  ...
  def ensure_can_create_rfq
    ...
    unless parts_request.status == 'procurement_notified'
